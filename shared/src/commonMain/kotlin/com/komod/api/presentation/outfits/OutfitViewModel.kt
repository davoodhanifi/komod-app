...
class OutfitViewModel(
    ...
    // Called when the Outfit screen resumes (e.g. returning from the system Settings app).
    // Only re-checks when we're actually blocked on permission, so a plain screen revisit
    // doesn't re-trigger a location/weather fetch or any permission prompt.
    fun recheckWeatherPermission() {
        if (shouldRecheckWeatherPermission(_uiState.value.weatherUiState)) {
...
// Extracted so the "when do we re-check?" decision is unit-testable on its own,
// independent of OutfitViewModel's platform-bound constructor dependencies
// (WeatherLocationService/WeatherPreferences/AppSettingsOpener have no fake-friendly
// interface and require a real Context on Android).
